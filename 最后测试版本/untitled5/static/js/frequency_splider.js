...
//异步处理，添加任务
function addTask1() {
    var ajaxCallUrl = hostname + "/message1";
    $.ajax({
        cache: true,
        type: "POST",
        url: ajaxCallUrl,
...
function showTask1(num) {
    ...
    var url = hostname + '/json1?status=' + num + '&type=4';
    $.ajax({
        type: "get",
        url: url,
        dataType: 'json',
            ...
            if (num == 0) {
                ...
                $.each(data['objects'], function (index, n) {
                    ...
                    $("#spliding").append(
                        ...
                        "<i class=\"fa fa-stop text-red\" style=\"color: red\"></i>" +
                        "</button>" +
                        "</div>" +
                        "<button type=\"button\" class=\"btn btn-default\" onclick='deleteTask(" + n.id + ")'>" +
