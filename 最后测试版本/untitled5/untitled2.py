...
from com.model.Food import Food
from com.model.Hotel import Hotel
from com.model.Senic_spot import Senic_spot
...
def search():
    ...
    if args == "hotel":
        result = Hotel.query.filter(Hotel.id<20).all()
        return to_json(result)
    elif args == "senic_spot":
        result = Senic_spot.query.filter(Senic_spot.id<20).all()
        return to_json(result)
    elif args == "food":
        result = Food.query.filter(Food.id<20).all()
        return to_json(result)
...
def deletebyid():
    ...
    print id
    if tablename == "hotel":
        result = Hotel.query.filter_by(id=id).first()
    elif tablename == "senic_spot":
        result = Senic_spot.query.filter_by(id=id).first()
    elif tablename == "food":
